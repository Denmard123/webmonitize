...
let { mattresses, testimonials, portfolios } = require('./data'); // Impor data dari data.js
...
app.delete('/api/mattresses/:id', (req, res) => {
    const { id } = req.params;
    mattresses = mattresses.filter(m => m.id != id);
    res.status(204).send();
});
...
// CRUD Endpoints untuk portfolio
app.get('/api/portfolios', (req, res) => {
    res.json(portfolios);
});

app.post('/api/portfolios', (req, res) => {
    const newPortfolio = { id: portfolios.length + 1, ...req.body };
